...
test('every published page loads one shared portal shell from the correct root', () => {
  for (const [relative, prefix] of pages) {
    ...
    const css = `${prefix}portal-shell.css?v=portal-20260913`;
    const js = `${prefix}portal-shell.js?v=portal-20260913`;
...
test('portal shell exposes all routes and motion-safe controls', () => {
  ...
  for (const route of ['./', 'room/', 'snapshot/', 'forschung/figuren-und-perspektiven/', 'forschung/formen-und-verbindungen/', 'forschung/transaktionsfluss/', 'forschung/q-notizen/', 'forschung/tagesstand-2026-09-13/']) {

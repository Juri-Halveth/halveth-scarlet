...
const words=[L.t('LIEBE BLEIBT'),L.t('FRAG DIE QUELLE'),L.t('ZUHÖREN IST MUT'),L.t('NOCH EIN BLICK'),L.t('VERTRAUEN WÄCHST'),L.t('HUMOR HILFT'),L.t('WISSEN TEILEN')],tones=['#f6a5d9','#b8a7ff','#9dffe1','#ffd99a'];
...
function paintSound(){soundButton.setAttribute('aria-pressed',String(soundEnabled));soundButton.textContent=soundEnabled?L.t('♫ Ton an'):L.t('♫ Ton aus');soundStatus.textContent=soundEnabled?L.t('Leise Spielklänge sind eingeschaltet.'):L.t('Ton startet erst nach deinem Klick.');}
...
async function toggleSound(){
  ...
  catch{soundEnabled=false;paintSound();soundStatus.textContent=L.t('Ton ist in diesem Browser nicht verfügbar.');}
}
...
function renderNews(now){if(!stories.length)return;const wire=document.querySelector('.newswire');if(wire.dataset?.newsSource==='feed'||wire.matches(':hover')||wire.contains(document.activeElement))return;const index=Math.floor(now/14000)%stories.length;if(index===newsIndex)return;newsIndex=index;const item=stories[index];newsTitle.textContent=item.title;newsTitle.href=item.url;newsMeta.textContent=item.publisher+' · '+item.date+L.t(' · Quellenstand 12.09.2026');}
...
function tick(){
  ...
  if(prelude){eventLabel.textContent=L.t('1-STUNDEN-SPIELCOUNTDOWN');eventTime.textContent=state.countdown;eventNote.textContent=L.t('Finale um ')+state.nextLabel+' · Berlin';}
  else{eventLabel.textContent=muted?L.t('FÜR DIESE RUNDE ALLES GRÜN'):L.t('NÄCHSTES SPIELEVENT');eventTime.textContent=state.nextLabel;eventNote.textContent=play.checked?L.t('Eine Stunde vorher, ab ')+state.opensLabel+' · Berlin':L.t('Automatische Spielevents ausgeschaltet');}
  ...
  if(scene.dataset.phase==='mischief'){
    ...
    if(step!==storyStep){storyStep=step;document.querySelector('#moment-line').textContent=[L.t('AHAA… DA SIND NEWS.'),L.t('BÖSE NEWS? ERST MAL LESEN.'),L.t('EIN STEIN. EINE NEUE SICHT.'),L.t('HEHEHE… EIN HERZ FEHLT NOCH.')][step];document.querySelector('#moment-label').textContent=preview?L.t('SPIELVORSCHAU · SCARLET'):L.t('SPIELFINALE · SCARLET');document.querySelector('#moment-note').textContent=L.t('Fang die Stimmung mit einem Herz auf.');}
